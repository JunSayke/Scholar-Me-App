...
public class UnMarkFavoriteCourseHandler implements Route {
    @Override
    public Object handle(Request req, Response res) throws Exception {
        ...
        try {
            ...
            try (Connection conn = MySQLConnection.getConnection();
                 PreparedStatement stmt = conn.prepareStatement("DELETE FROM tbluserfavorite WHERE courseid = ? AND userid = ?")) {
                stmt.setInt(1, Integer.parseInt(req.queryParams("courseId")));
                stmt.setInt(2, req.attribute("userId"));
                if (stmt.executeUpdate() == 0) {
                    throw new InvalidFieldException(400, "Course not found in favorite list");
                }
            }
            res.status(200);
            return GsonData.objectToJson(new ResponseGson<>(true, "Course unmarked as favorite successfully"));
